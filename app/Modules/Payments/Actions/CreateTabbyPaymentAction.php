...
class CreateTabbyPaymentAction
{
    public function __construct(private readonly TabbyPaymentService $tabby) {}
    ...
    public function execute(Invoice $invoice): array
    {
        ...
        $session = $this->tabby->createPayment($invoice->load('customer'));
        $transaction = PaymentTransaction::query()->create([
            ...
            'raw_payload' => $session['raw'] ?? null,
        ]);

        return ['payment_url' => $session['payment_url'], 'session' => $session, 'transaction' => $transaction];
    }
}
